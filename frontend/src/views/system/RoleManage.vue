<template>
  <div class="page-container">
    ...
    <el-dialog v-model="menuDialogVisible" title="分配菜单" width="420px" destroy-on-close>
      <el-tree
        ref="menuTreeRef"
        :data="menuTree"
...
const handleSave = async () => {
  if (form.id) { await updateRole(form) } else { await createRole(form) }
...
const handleAssignMenu = async (row) => {
  currentRoleId.value = row.id; menuDialogVisible.value = true
  await loadMenuTree()
  const res = await getRoleMenus(row.id)
  if (res.code === 200) { nextTick(() => { menuTreeRef.value?.setCheckedKeys(res.data.map(m => m.id)) }) }
}
const handleSaveMenus = async () => {
  const checkedKeys = menuTreeRef.value?.getCheckedKeys() || []
  await assignMenus(currentRoleId.value, checkedKeys)
  menuDialogVisible.value = false; ElMessage.success('分配成功')
}
onMounted(() => { loadData() })
</script>
